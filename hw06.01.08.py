...
with open('hw06.01.08.txt','r') as infile:
    data = []
    for line in infile:
        data.extend(line.strip().split())
intData = sorted([float(strValue) for strValue in data])
print(intData)
length = len(intData)
print('found %d items.' % length)
...
width = round((dataCeil - dataFloor + .004) / catCount, 3)
